...
class LaneFilterHistogramKF():
    ...
    def __init__(self, **kwargs):
        ...
        self.Q = np.array([[0.05, 0], [0, 0.20]]) # Q is the noise covariance matrix --> to be tuned
        self.matrix = 0
    ...
    def predict(self, dt, left_encoder_delta, right_encoder_delta):
        ...
        F = self.Fmatrix(alpha, left_encoder_delta, right_encoder_delta, phi_belief)
    ...
    def Fmatrix(self, alpha, left_encoder_data, right_encoder_data, phi_belief):
        F = np.eye((2))
        F[0, 1] = np.cos(phi_belief) * self.wheel_radius * (left_encoder_data + right_encoder_data) * 0.5 / alpha
        return F
    ...
    def update(self, segments):
        ...
        if measurement_likelihood is not None:
            ...
            # simplified operations as H, M are Id 
            V = P_predicted + sigma
            K = P_predicted @ np.linalg.inv(V)
            X_updated = X_predicted + K @ (mean_vec-X_predicted)
            P_updated = P_predicted - K @ P_predicted
    ...
    def getSigmaMatrix(self, distribution_matrix, mean_value, i_min, j_min, i_max, j_max):
        ...
        for i in range(i_min, i_max):
            ...
            for j in range(j_min, j_max):
                phi_i = self.phi_min + (j + 0.5) * self.delta_phi
                X = np.array([[d_i], [phi_i]])-mean_value
                sigma += distribution_matrix[i, j] * X @ X.T
